import React from 'react';
import './app.css';
import setting from './assets/image/setting.png';
import swiper1 from './assets/image/swiper-1.png';
import swiper2 from './assets/image/swiper-2.png';
import swiper3 from './assets/image/swiper-3.png';

function app() {

}

class App extends React.Component {
    constructor(props){
        super(props);

        this.state = {
            agentList: [],
            navList: [
                {code:'1',icon:"icon icon-dashboard",name:'DASHBOARD',path:"/app/dashboard"},
                {code:'2',icon:"icon icon-sitemap",name:'AGENT',path:"/app/agent"},
                {code:'3',icon:"icon icon-boat",name:'MY CRUISE',path:"/app/myCruise"},
                {code:'4',icon:"icon icon-life-bouy",name:'HELP',path:"/app/help"}
            ],
            navSelection: this.props.location.pathname,
            sidebarDisplay: document.body.clientWidth < 1024 ? false : true
        }

    }

    sideClick = (item) => {
        if(document.body.clientWidth < 1024){
            this.setState({
                navSelection: item.path,
                sidebarDisplay: false
            })
        } else {
            this.setState({
                navSelection: item.path
            })
        }
        this.props.history.push(item.path);
    }

    sideClose = () => {
        this.setState({
            sidebarDisplay: false
        })
    }

    sidebarShow = () => {
        this.setState({
            sidebarDisplay: true
        })
    }

    componentDidMount() {
        window.addEventListener('resize', () => {
            if(document.body.clientWidth < 1024){
                this.setState({
                    sidebarDisplay: false
                })
            } else {
                this.setState({
                    sidebarDisplay: true
                })
            }
        })

        api.getAgentList().then((result) => {
            this.setState({
                agentList: result
            })
        })
    }

    render() {
        return (
            <div className="app">
                <div className="title">
                    <span>安全指挥中心</span>
                    <span className="tag">全国</span>
                </div>
                <ul className="container">
                    <li className="container-left three">
                        <div className="item">
                            <div className="item-title">
                                <div className="text-left">告警轮播</div>
                            </div>
                            <table className="table">
                                <thead className="thead">
                                    <tr className="tr">
                                        <td className="td text-left">区域</td>
                                        <td className="td text-left">告警类型</td>
                                        <td className="td text-right">时间</td>
                                    </tr>
                                </thead>
                                <tbody className="tbody">
                                    <tr className="tr" style={{backgroundImage:"url("+swiper1+")"}}>
                                        <td className="td text-left">走廊</td>
                                        <td className="td text-left">人员异常聚集告警</td>
                                        <td className="td text-right">2019-05-12 21:03</td>
                                    </tr>
                                    <tr className="tr" style={{backgroundImage:"url("+swiper2+")"}}>
                                        <td className="td text-left">走廊</td>
                                        <td className="td text-left">人员异常聚集告警</td>
                                        <td className="td text-right">2019-05-12 21:03</td>
                                    </tr>
                                    <tr className="tr" style={{backgroundImage:"url("+swiper3+")"}}>
                                        <td className="td text-left">走廊</td>
                                        <td className="td text-left">人员异常聚集告警</td>
                                        <td className="td text-right">2019-05-12 21:03</td>
                                    </tr>
                                </tbody>
                            </table>
                        </div>
                        <div className="item">
                            <div className="item-title">
                                <div className="text-left">安全监控</div>
                                <div className="text-right">
                                    <img src={setting} width="16" height="16"></img>
                                    <span>全部</span>
                                </div>
                            </div>
                            <ul className="pie-list">
                                <li className="pie">
                                    <div id="par1"></div>
                                </li>
                                <li className="pie"></li>
                                <li className="pie"></li>
                                <li className="pie"></li>
                                <li className="pie"></li>
                                <li className="pie"></li>
                            </ul>
                        </div>
                        <div className="item"></div>
                    </li>
                    <li className="container-content"></li>
                    <li className="container-right three">
                        <div className="item"></div>
                        <div className="item"></div>
                        <div className="item"></div>
                    </li>
                </ul>
            </div>
          );
    }
}

export default app;
